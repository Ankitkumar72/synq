...
final taskStatsProvider = Provider<AsyncValue<TaskStats>>((ref) {
  ...
  return notesAsync.whenData((notes) {
    ...
    for (final task in todayTasks) {
      final isRecurring =
          task.parentRecurringId != null || task.recurrenceRule != null;
      if (!isRecurring) {
        nonRecurringTotal++;
      ...
      recurringSeriesCompletion[seriesId] =
          alreadyCompleted || task.isCompleted;
    }

    final recurringTotal = recurringSeriesCompletion.length;
    final recurringCompleted = recurringSeriesCompletion.values
        .where((isDone) => isDone)
        .length;
